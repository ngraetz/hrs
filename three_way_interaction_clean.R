...
formula2 <- cognitive ~ as.factor(female) + as.factor(cohort_group) + center_age*over_70race + 
...
## Make Table 1
library(survey)
model_hrs[, race := as.character(race)]
des <- svydesign(id=~id, weight=~pweight, data=model_hrs[!is.na(cognitive), ])
get_table1 <- function(v) {
  p <- svyttest(formula=as.formula(paste0(v,'~race')), design=des)$p.value
  p <- data.table(type='pvalue',mean=p,se=0)
  race_means <- as.data.table(svyby(as.formula(paste0('~',v)), ~race, design=des, svymean, na.rm.all = T))
  setnames(race_means, c('type','mean','se'))
  full_mean <- as.data.table(svymean(as.formula(paste0('~',v)), design=des))
  full_mean[, type := 'full']
...
table1 <- rbindlist(lapply(c('original_baseline_cog','original_baseline_income','original_baseline_wealth',
                             'edu_years','edu_years_mother','age'), get_table1))
...
formula4 <- as.formula(paste0(outcome_var, ' ~ matchvec_1 + center_age*edu_years*race + (center_age||id_factor)'))
...
coef_table <- as.data.table(expand.grid(sort(unique(model_hrs[, center_age])), c('white','black'), c(-1.903866,-0.3609927,1.181881)))
setnames(coef_table, c('center_age','race','edu_years'))
coef_table[, raceblack := ifelse(race=='black',1,0)]
## EDU * AGE DUMMIES
  coef_table[, (paste0('center_age:edu_years')) := center_age * edu_years]
## EDU * RACE DUMMIES
  coef_table[, (paste0('edu_years:raceblack')) := edu_years * raceblack]
## RACE * AGE DUMMIES
  coef_table[, ('center_age:raceblack') := center_age * raceblack]
## EDU * AGE * RACE DUMMIES
  coef_table[, (paste0('center_age:edu_years:raceblack')) := center_age * edu_years * raceblack]
...
doc <- read_docx() %>%
  ...
  print(target = paste0("table1", file_tag, ".docx"))
...
clean_names <- data.table(variable=c("(Intercept)","as.factor(female)1","as.factor(cohort_group)1940","as.factor(cohort_group)1950","baseline_wealth","baseline_income","baseline_cog","edu_years","edu_years_mother","center_age","raceblack",
                          ...
                          name=c('Intercept','Female','1940 cohort','1950 cohort','Wealth (baseline)','Income (baseline)','Cognition (baseline)','Years of education','Mother years of education','Age','Black','Age*Edu','Age*Black','Edu*Black','Age*Edu*Black','Age*MEdu','MEdu*Black','Age*MEdu*Black','Marginal R2','Conditional R2','BIC','Person-years','Individuals'))
clean_names[, cov_sort := 1:.N]
coefs <- merge(coefs, clean_names, by='variable')
...
ft <- flextable(coefs, theme_fun = theme_booktabs) %>%
  ...
  hline(j=1:13,i=14,part = 'body', border = fp_border(color='black',width=2)) %>%
  padding(padding = 0.0001, part = 'all') %>%
  padding(padding.top = 0, part='all') %>%
  # width(j=2:8, width=0.4) %>%
  # width(j=1, width=2.23) %>%
...
doc <- read_docx() %>%
  body_add_flextable(value = ft, split = TRUE) %>%
  body_end_section_landscape() %>% # a landscape section is ending here
  print(target = paste0("table2_", file_tag, ".docx"))
